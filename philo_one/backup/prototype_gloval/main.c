...
/*   Updated: 2020/11/06 10:14:40 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*work(void *args)
{
	...
	while (1)
	{
		...
		pthread_mutex_lock(&g_m_fork[(phs->num + 1) % phs->params->number_of_phs]);
		gettimeofday(&now, NULL);
		write_message(convert_to_milliseconds(now), FORK, phs);
		// 食事
		phs->start_time_to_eat = now;
		...
		pthread_mutex_unlock(&g_m_fork[(phs->num + 1) % phs->params->number_of_phs]);
		if (phs->params->flag_set_max_number_of_eat == true && phs->num_of_eat == phs->params->max_number_of_eat)
			break ;
		// 睡眠をとる
		gettimeofday(&now, NULL);
