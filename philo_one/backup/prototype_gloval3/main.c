...
/*   Updated: 2020/11/08 02:43:18 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			start_simulation(void)
{
	...
	if ((phs = malloc(sizeof(t_philosopher) * g_params.number_of_phs)) == NULL)
		return (ERROR);
	if ((threads = malloc(sizeof(pthread_t) * g_params.number_of_phs)) == NULL)
	{
		free(phs);
		return (ERROR);
	}
	if ((g_params.m_fork = malloc(sizeof(pthread_mutex_t) * g_params.number_of_phs)) == NULL)
	{
		free(phs);
		free(threads);
		return (ERROR);
	...
	i = 0;
	while (i < g_params.number_of_phs)
	{
		pthread_mutex_init(&g_params.m_fork[i], NULL);
		i++;
	}
	i = 0;
	while (i < g_params.number_of_phs)
	{
		if (init_t_philosopher(&phs[i], i))
		{
			free(phs);
			free(threads);
			free(g_params.m_fork);
			return (ERROR);
		}
		i++;
	}
	...
	while (i < g_params.number_of_phs)
	{
		pthread_mutex_destroy(&g_params.m_fork[i]);
		i++;
	}
	free(g_params.m_fork);
	free(phs);
