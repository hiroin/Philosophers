...
/*   Updated: 2020/11/05 02:11:22 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			start_simulation(t_params params)
{
	...
	phs->params->m_fork = malloc(sizeof(pthread_mutex_t) * params.number_of_phs);
	i = 0;
	while (i < params.number_of_phs)
	{
		pthread_mutex_init(&phs->params->m_fork[i], NULL);
		i++;
	}
	i = 0;
	while (i < params.number_of_phs)
	...
	while (i < params.number_of_phs)
	{
		pthread_mutex_destroy(&phs->params->m_fork[i]);
		i++;
	}
	free(phs);
	free(threads);
