...
/*   Updated: 2020/11/13 07:07:26 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	death_confirmation(struct timeval now, t_philosopher *phs)
{
	...
	result = diff_time(now, phs->start_time_to_eat) >= phs->params->time_to_die;
...
int			start_simulation(t_params params)
{
	...
	params.sem_other = sem_open(SEMNAME_OTHER, O_CREAT , 0600, 1);
	if (params.sem_other == SEM_FAILED)
	{
		sem_close(params.sem_forks);
	...
	while (i < params.number_of_phs)
	{
		...
		if (pid == 0)
		{
			...
			while (1)
			{
				sem_wait(params.sem_other);
				if (phs.exit_code != -1)
				{
					sem_post(params.sem_other);
					break;
				}
				sem_post(params.sem_other);
			}
